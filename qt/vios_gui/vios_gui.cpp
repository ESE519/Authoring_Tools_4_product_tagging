    ...
    ui(new Ui::ViosGui)
{
    ...
    connect(ui->obj_fwd, SIGNAL(clicked()), signalMapper, SLOT(map()));
    signalMapper->setMapping(ui->fwd_skip_button, QString::fromStdString("obj_fwd"));
    connect(ui->obj_fwd_skip, SIGNAL(clicked()), signalMapper, SLOT(map()));
    signalMapper->setMapping(ui->prev_skip_button, QString::fromStdString("obj_fwd_skip"));
    connect(ui->obj_prev, SIGNAL(clicked()), signalMapper, SLOT(map()));
    signalMapper->setMapping(ui->fwd_button, QString::fromStdString("obj_prev"));
    connect(ui->obj_prev_skip, SIGNAL(clicked()), signalMapper, SLOT(map()));
    signalMapper->setMapping(ui->prev_button, QString::fromStdString("obj_prev_skip"));
    ...
    scene_obj=new QGraphicsScene();
    scene_cropped_obj=new QGraphicsScene();
    ...
    ui->obj_graphicsView->setScene(scene_obj);
    ui->detected_obj_view->setScene(scene_cropped_obj);
    ...
    train_path = "home/shashidhar/Documents/Authoring_Tools_4_product_tagging/qt/train/";
    junk_path = "/home/shashidhar/Documents/Authoring_Tools_4_product_tagging/qt/train/junk";
...
void ViosGui::obj_img_update(){
    ...
    image_qt = Mat2QImage(image_cv);
    scene_obj->clear();
    scene_obj->addPixmap(QPixmap::fromImage(image_qt));
}
