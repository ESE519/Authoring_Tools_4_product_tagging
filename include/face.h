	...
	class FaceTracker: public FaceRecognize{
	  public:
		...
		public:
			...
			//~FaceTracker();
			void klt_face_tracker(vector <string> &name, vector <int> &label, vector <Rect> &face_pos, Mat &img, Mat &prev_img, vector < vector <Point2f> > &features, string classifier = "fisher", string mode = "front");
	};
}
